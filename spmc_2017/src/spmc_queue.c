...
int spmc_queue_init(spmc_queue_t *queue, int argc, char *argv[]) {
    ...
    queue->q.head = 0;
    queue->q.tail = 0;
    queue->q.lastItemDequeued = -1;
    queue->q.cells = (spmc_cell_t*)malloc(size * sizeof(spmc_cell_t));
    for (int i = 0; i < size; ++i) {
        queue->q.cells[i].rank = EMPTY_CELL;
        queue->q.cells[i].gap = 0;
        queue->q.cells[i].data = EMPTY_CELL;
    }

// Tạo MPI window cho cells, head, tail
    MPI_TRY(mpi_win_create(&queue->q, mpi_is_root(&queue->mpi_ctx) ? size * sizeof(spmc_cell_t) : 0, sizeof(spmc_cell_t), queue->mpi_ctx.comm, &queue->win_queue));
...
void spmc_queue_destroy(spmc_queue_t *queue) {
    ...
    mpi_free(queue->q.cells, 0, mpi_get_rank(&queue->mpi_ctx));
    mpi_finalize();
