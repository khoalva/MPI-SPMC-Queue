...
int main(int argc, char *argv[]) {
    ...
    MPI_EASY_TRY(mpi_easy_bcast(&broadcast_value, 1, MPI_EASY_INT, 0, ctx.comm));
    ...
    int global_sum = 0;
    MPI_EASY_TRY(mpi_easy_reduce(&local_sum, &global_sum, 1, MPI_EASY_INT, 
                                 MPI_EASY_SUM, 0, ctx.comm));
    ...
    int global_max = 0;
    MPI_EASY_TRY(mpi_easy_allreduce(&local_max, &global_max, 1, MPI_EASY_INT, 
                                    MPI_EASY_MAX, ctx.comm));
    ...
    MPI_EASY_TRY(mpi_easy_allreduce(local_data, global_averages, ARRAY_SIZE, 
                                    MPI_EASY_INT, MPI_EASY_SUM, ctx.comm));
